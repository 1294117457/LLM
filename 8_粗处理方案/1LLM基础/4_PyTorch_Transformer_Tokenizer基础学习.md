...
- [ ] Tensor 是什么？为什么要放到 GPU 上？
  - [ ] 张量，多维数组；利用GPU加速，加速矩阵计算

- [ ] `loss.backward()` 做了什么？为什么训练时 loss 会下降？
  - [ ] 反向推算精准度，推算计算参数的梯度；loss下降因为优化器顺梯度反方向更新模型参数

- [ ] Self-Attention 解决了什么问题？Q-K-V 分别代表什么？
  - [ ] token之间从只能从后往前关注转为互相关注，解决了长距离上下文依赖问题
  - [ ] 输入的tokenIDs经过问题，关键词，值权重矩阵计算得出的矩阵

- [ ] GPT/Qwen 为什么是"一个字一个字生成"的？
  - [ ] **自回归模型（Auto-regressive）**。
  - [ ] 每次计算只能基于当前全部上文预测出下一个概率最高的 token，
  - [ ] 将其拼接进历史上下文后，才能进入下一轮循环
...
- [ ] `apply_chat_template` 在做什么？为什么微调时需要它？
  - [ ] 将messages转为模型认识的拼接文本

- [ ] Embedding 是什么？token ID 怎么变成模型能处理的向量？
  - [ ] **mbedding 是查表映射**，
  - [ ] 把离散的 Token ID（如 `151644`）转换为包含实际语义和数值的浮点数多维矩阵（如 `896` 维向量），
  - [ ] 使得神经网络可以进行真正的数学运算。
